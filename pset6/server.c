...
char* indexes(const char* path)
{
    ...
    struct dirent *d;
    int len = strlen(path) + strlen("index.php");

    while(directory)
    {
        if((d = readdir(directory)) != NULL)
        {
            if(strcmp(d->d_name, "index.php") == 0)
            {
            ...
            {
                char* dd = malloc((len+1)*sizeof(char));
                strcpy(dd, path);   
                return strcat(dd, "index.html");
            }
        }
        else
...
const char* lookup(const char* path)
{
    ...
    {
        char* tmp = malloc(sizeof(char)*(strlen(path)+2));
    ...
    else
    {
        ...
        // printf("last dot = %s\n", last_dot);
        char* tmp = malloc(sizeof(char)*(strlen(last_dot)+1));
        strncpy(tmp, last_dot, strlen(last_dot));
        *(tmp + strlen(last_dot)) = '\0';
  
      
        // printf("abs_path =%s, last b =%s\n", path, tmp);
